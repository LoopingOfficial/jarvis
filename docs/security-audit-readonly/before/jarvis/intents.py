...
def detect_read_only_intent(text: str) -> ReadOnlyIntent:
    """Détecte le mode lecture seule de manière déterministe, avant le LLM."""
    value = (text or "").strip()
    constrained = bool(_READ_ONLY_CONSTRAINT.search(value))
    requested = bool(_READ_ONLY_REQUEST.search(value))
    read_only = constrained or requested
    security = bool(_SECURITY_REQUEST.search(value)) and (read_only or requested)
    return ReadOnlyIntent(
        intent="security_analysis" if security else "read_only" if read_only else "general",
        read_only=read_only,
        write_allowed=not read_only,
        explicit_constraint=constrained,
        target=(_ANALYSIS_TARGET.search(value).group(1) if _ANALYSIS_TARGET.search(value) else ""),
    )
