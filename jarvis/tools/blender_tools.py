...
def _animate(ctx: ToolContext) -> ToolResult:
    ...
    if unknown:
        return ToolResult(
            ...
            + ". Clips d'objet disponibles : " + ", ".join(sorted(SUPPORTED))
            + ". Les clips de personnage (idle, walk, wave...) ne sont pas encore pris en charge "
            "sur ce poste.", risk=SAFE_WRITE)
...
registry.add(
    ...
    description=(
        ...
        "Clips d'objet (sans rig) disponibles : spin, rotate, turn, twirl, float, "
        "bounce, orbit, pulse. Repond a « fais-la tourner », « fais-la rebondir ». "
        "ATTENTION : les clips de personnage (walk, wave, sit...) ne sont PAS encore "
        "implémentés : ne les promets pas a l'utilisateur."),
    ...
    input_schema={
        ...
        "properties": {
            "clip": {"type": "string",
                     "description": "Spin, rotate, turn, twirl, float, bounce, orbit, pulse."},
            "clips": {"type": "array", "items": {"type": "string"},
                      "description": "Plusieurs clips d'un coup."},
            "duration": {"type": "number", "description": "Duree en secondes (0 = defaut)."},
            "fps": {"type": "integer"},
            "axis": {"type": "string", "enum": ["X", "Y", "Z"],
                     "description": "Axe pour les clips d'objet."},
            "amplitude": {"type": "number"},
            "target": {"type": "string", "description": "Objet vise pour un clip d'objet."},
            "replace": {"type": "boolean",
                        "description": "Supprimer les animations existantes d'abord."},
            "options": {"type": "object",
                        "description": "Options du clip : side (L/R), angle, stride."},
        },
        "required": [],
    },
)
