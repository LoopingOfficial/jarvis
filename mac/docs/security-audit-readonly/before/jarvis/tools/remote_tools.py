...
def _ssh_read_file(ctx: ToolContext) -> ToolResult:
    ...
    active = dict(getattr(ctx.core, "active_task_context", {}) or {})
    if root and not active.get("remote_root"):
        active["remote_root"] = root
    try:
...
def _db_query(ctx: ToolContext) -> ToolResult:
    ...
    if ctype == "mysql":
        inner = (f"mysql -h {shell_quote(str(cfg.get('host', '127.0.0.1')))} "
                 f"-P {int(cfg.get('port') or 3306)} -u {shell_quote(str(cfg.get('username', '')))} "
                 f"{'-p' + shell_quote(password) if password else ''} "
