...
from .mail_providers import PROVIDER_KEYS, PROVIDER_LABELS
...
class FieldSpec:
    ...
    placeholder: str = ""
    options: tuple[str, ...] = ()
    options_labels: tuple[str, ...] = ()   # libellés affichés (sinon = options)
    default: Any = ""
    help: str = ""
...
_register(ConnectorType(
    type="email", label="Email (SMTP + IMAP)", category="Communication", icon="mail",
    description=("Une seule boîte pour envoyer (SMTP) et lire (IMAP). Choisis un fournisseur "
                 "(Gmail, Outlook/Hotmail, iCloud, Yahoo…) : hôte et port sont déduits, il ne "
                 "reste qu'à coller un mot de passe d'application."),
    default_permissions=("read", "write", "execute"),
    fields=(
        _f("provider", "Fournisseur", kind="select", options=PROVIDER_KEYS,
           options_labels=PROVIDER_LABELS, default="custom",
           help="Gmail/Outlook/iCloud/Yahoo pré-remplissent hôtes et ports automatiquement."),
        _f("email", "Adresse email", required=True, placeholder="vous@exemple.com"),
        _f("username", "Identifiant", placeholder="Défaut = adresse email"),
        _f("password", "Mot de passe d'application", kind="password", secret=True,
           help="Un mot de passe d'application, pas ton mot de passe habituel."),
        _f("from_name", "Nom d'expéditeur", placeholder="Ex. : Atelier Martin"),
        _f("from_address", "Adresse d'envoi", placeholder="Défaut = adresse email"),
        _f("imap_host", "Hôte IMAP (réception)", placeholder="imap.gmail.com"),
        _f("imap_port", "Port IMAP", kind="number", default=993),
        _f("imap_ssl", "IMAP en SSL", kind="bool", default=True),
        _f("smtp_host", "Hôte SMTP (envoi)", placeholder="smtp.gmail.com"),
        _f("smtp_port", "Port SMTP", kind="number", default=587),
        _f("smtp_tls", "SMTP STARTTLS", kind="bool", default=True),
    ),
))
        ...
        out.append({
            ...
            "fields": [
                {"key": f.key, "label": f.label, "kind": f.kind, "required": f.required,
                 ...
                 "options_labels": list(f.options_labels), "default": f.default, "help": f.help}
                for f in ct.fields
            ],
        })
...
class ConnectorManager:
    ...
    def set_enabled(self, connector_id: str, enabled: bool,
                    llm: Any = None) -> dict[str, Any] | None:
        self._db.execute(
            "UPDATE connectors SET enabled=?, updated_at=? WHERE id=?",
            (1 if enabled else 0, time.time(), connector_id),
        )
        if not enabled:
            raw = self.raw(connector_id)
            if raw and raw.get("type") == "ollama":
                self._unload_ollama_models(raw)
        if llm is not None:
            llm.invalidate()
        self._events.emit("connector.updated", {"id": connector_id, "enabled": bool(enabled)})
        return self.get(connector_id)

    def _unload_ollama_models(self, raw: dict[str, Any]) -> None:
        """Décharge les modèles Ollama de la VRAM quand le connecteur est désactivé."""
        import json as _json
        import threading as _threading
        import urllib.request

        base_url = str((raw.get("config") or {}).get("base_url") or "http://127.0.0.1:11434").rstrip("/")

        def _do_unload():
            try:
                with urllib.request.urlopen(base_url + "/api/ps", timeout=4) as resp:
                    data = _json.loads(resp.read())
                models = [m.get("name") for m in (data.get("models") or []) if m.get("name")]
                for name in models:
                    body = _json.dumps({"model": name, "keep_alive": 0}).encode()
                    req = urllib.request.Request(
                        base_url + "/api/generate", body,
        ...
        _threading.Thread(target=_do_unload, daemon=True, name="jarvis-ollama-unload").start()
    ...
    def _run_test(self, c: dict[str, Any]) -> tuple[bool, str]:
        ...
        if ctype in {"smtp", "imap", "email"}:
            from .mail import test_connection

            return test_connection(ctype, cfg, sec("password"))
        ...
        if ctype in {"discord", "slack"}:
            ...
            token = sec("bot_token")
            if ctype == "discord" and token:
                channel_id = str(cfg.get("default_channel") or "").strip()
                headers = {"Authorization": f"Bot {token}"}
                ok, user = http_json("https://discord.com/api/v10/users/@me",
                                     headers=headers, timeout=12)
                if not ok or not isinstance(user, dict) or not user.get("id"):
                    return False, "Bot Discord non authentifié."
                if not channel_id.isdigit():
                    return True, "Bot Discord authentifié (salon non vérifié)."
                ok, channel = http_json(f"https://discord.com/api/v10/channels/{channel_id}",
                                        headers=headers, timeout=12)
                if not ok or not isinstance(channel, dict) or not channel.get("guild_id"):
                    return False, "Salon Discord inaccessible."
                guild_id = str(channel["guild_id"])
                ok, guild = http_json(f"https://discord.com/api/v10/guilds/{guild_id}",
                                      headers=headers, timeout=12)
                if not ok or not isinstance(guild, dict):
                    return False, "Serveur Discord inaccessible."
                ok, member = http_json(f"https://discord.com/api/v10/guilds/{guild_id}/members/{user['id']}",
                                       headers=headers, timeout=12)
                ok_roles, roles = http_json(f"https://discord.com/api/v10/guilds/{guild_id}/roles",
                                            headers=headers, timeout=12)
                if not ok or not ok_roles or not isinstance(member, dict) or not isinstance(roles, list):
                    return False, "Membre ou rôles du serveur inaccessibles."
                role_map = {str(r.get("id")): r for r in roles}
                everyone = role_map.get(guild_id, {})
                permissions = int(everyone.get("permissions") or 0)
                for role_id in member.get("roles") or []:
                    permissions |= int((role_map.get(str(role_id)) or {}).get("permissions") or 0)
                admin = bool(permissions & 0x8)
                allow, deny = 0, 0
                for overwrite in channel.get("permission_overwrites") or []:
                    oid = str(overwrite.get("id"))
                    if oid == guild_id:
                        deny |= int(overwrite.get("deny") or 0)
                        allow |= int(overwrite.get("allow") or 0)
                    elif oid in {str(x) for x in (member.get("roles") or [])}:
                        deny |= int(overwrite.get("deny") or 0)
                        allow |= int(overwrite.get("allow") or 0)
                effective = permissions if admin else (permissions & ~deny) | allow
                checks = {"ViewChannel": admin or bool(effective & 0x400),
                          "SendMessages": admin or bool(effective & 0x800),
                          "EmbedLinks": admin or bool(effective & 0x4000)}
                if not all(checks.values()):
                    return False, "Salon accessible mais permissions insuffisantes (ViewChannel, SendMessages ou EmbedLinks)."
                return True, "Bot authentifié, serveur et salon accessibles; permissions requises OK."
