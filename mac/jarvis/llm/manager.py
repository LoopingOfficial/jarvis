...
class LLMManager:
    ...
    def chat_stream(
        self, messages: Any, *, role: str = "default", tools: list[dict[str, Any]] | None = None,
        temperature: float | None = None, max_tokens: int = 4096, timeout: float = 180.0,
        model_override: str = "", on_token=None,
    ) -> LLMResponse:
        """Generation token par token. Repli sur chat() si tools ou provider sans stream."""
        if tools:
            return self.chat(messages, role=role, tools=tools, temperature=temperature,
                             max_tokens=max_tokens, timeout=timeout,
        ...
        if model_override:
            ...
            if override_provider:
                provider, model = override_provider, (m or override_provider.default_model)
        if provider is None:
            return self.chat(messages, role=role, tools=None, temperature=temperature,
                             max_tokens=max_tokens, timeout=timeout,
                             model_override=model_override)
        ...
        if not callable(stream_fn):
            response = provider.chat(messages, model=model, tools=None,
                                     ...
                                     timeout=timeout)
            if response.ok and response.text and on_token:
                on_token(response.text)
            return response
        return stream_fn(messages, model=model, tools=None, temperature=temperature,
                         max_tokens=max_tokens, timeout=timeout, on_token=on_token)
