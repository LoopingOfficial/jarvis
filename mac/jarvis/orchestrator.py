...
from .fast_actions import FastActionRouter, tools_needed
...
# -- entrée principale -------------------------------------------------
    ...
    def _handle(
        ...
        # Fast path immediat : salutations, identite, nom. Avant MODEL_ONLY,
        # les taches et le LLM — aucune initialisation d'agent.
        if not attachment_ids:
            fast = self.fast_actions.execute(
                text, conversation_id, resolved_intent=resolved,
        ...
        if resolved.resource_type == "GOOGLE_SHEET":
            ...
            # --- Workspace immédiat -------------------------------------
            # Tout ce que l'interface affiche (tableaux, KPI, previews,
            # preuves, statistiques) provient de l'analyse déterministe, qui
            # est prête ICI. Attendre la synthèse du modèle pour ouvrir le
            # Workspace faisait patienter 20 à 30 s devant un écran vide alors
            # que les données étaient disponibles. On publie donc le payload
            # tout de suite, sans narration, puis on injecte la synthèse quand
            # elle arrive.
            preliminary = build_payload(
                request=original_text,
                source={"kind": "google_sheet", "label": _sheet_label(workbook, url),
                        "url": url},
                analysis=analysis,
                narrative="",
                grounding={},
            ...
            preliminary["summary"]["pending_narrative"] = True
            preliminary["summary"]["badge"] = ""
            timings["workspace_ready_ms"] = int((time.time() - started_at) * 1000)
            self._emit_sheet_workspace(preliminary, conversation_id, request_id,
                                       phase="deterministic")
            stage("sheet_workspace_ready",
                  ready_ms=timings["workspace_ready_ms"],
                  tables=sum(s["table_count"] for s in analysis["sheets"]))
            self._debug(f"[sheet] request_id={request_id} workspace_ready_ms="
                        f"{timings['workspace_ready_ms']} (synthèse en cours)", force=True)
            ...
            # La synthèse validée rejoint le Workspace déjà ouvert : on
            # n'envoie que le bloc « Synthèse », pas un second payload complet.
            self._emit_sheet_summary(payload, conversation_id, request_id)
            self._emit_sheet_workspace(payload, conversation_id, request_id,
                                       phase="final")
            # Gain réellement mesuré : délai avant premier affichage utile.
            timings["time_saved_ms"] = max(
                0, timings["total_ms"] - timings.get("workspace_ready_ms", 0))
        ...
        # Voie LLM directe : pas d'outil pertinent → pas de tache agent, streaming SSE.
        connector_preview = resolve_connector_intent(core, routing_text)
        if (not (connector_preview and connector_preview.executable)
                and not detect_image_intent(routing_text)
                and not detect_3d_intent(routing_text)
                and not tools_needed(routing_text)):
            return self._run_direct_llm(
                routing_text, conversation_id, execution_policy=execution_policy,
...
# 2. Boucle agentique complète, dans une tâche suivie
        ...
        *, request_id: str,
        ...
        for _ in range(3):
            ...
            response = self._stream_chat(
                messages, role=policy.get("model_role", "default"), tools=None,
    ...
    def _stream_chat(
        ...
        def on_token(chunk: str) -> None:
            ...
            if not started:
                started = True
                core.events.emit("jarvis.state", {"state": "SPEAKING", "reason": "llm_stream"})
            core.events.emit(
                "llm.delta",
                {"text": "".join(acc), "chunk": chunk, "conversation_id": conversation_id},
                cache=False,
            )
    ...
    def _run_direct_llm(
        self, text: str, conversation_id: str, *,
        execution_policy: dict[str, Any] | None = None, attachment_suffix: str = "",
    ) -> dict[str, Any]:
        """Reponse LLM sans outils, sans tache agent, avec streaming SSE."""
        core = self._core
        spec = AGENTS["jarvis"]
        core.events.emit("jarvis.state", {"state": "THINKING", "reason": "direct_llm"})
        messages = self._build_messages(
            spec, text, conversation_id, tools=[], skip_recall=True,
        ...
        if not response.ok:
            fallback = self._offline_fallback(text, response.error)
            core.conversations.add_message(
                conversation_id, "assistant", fallback, meta={"error": True, "direct_llm": True})
        ...
        core.conversations.add_message(
            conversation_id, "assistant", final_text,
            meta={"direct_llm": True, "tools": []})
        return {"ok": True, "response": final_text, "action": "fast.llm",
                "conversation_id": conversation_id, "tools_used": []}
    ...
    def _run_loop(
        ...
        need_tools = bool(intent and intent.executable) or tools_needed(text)
        ...
        for step in range(max_iterations):
            ...
            core.activity(title="Réflexion", detail="Analyse de la demande", kind="think",
                          state="THINKING")
            response = self._stream_chat(
                messages, role="default", tools=schemas or None,
                temperature=core.settings.get("ai", "temperature", 0.3),
                conversation_id=conversation_id)
            if not response.ok:
                core.tasks.fail(task_id, response.error)
    ...
    SHEET_STAGES = {
        ...
        # Le Workspace s'ouvre ICI : les donnees deterministes sont pretes,
        # la synthese du modele continue en arriere-plan.
        "sheet_workspace_ready": ("Workspace prêt", 5),
        "sheet_llm": ("Analyse des données", 6),
        "sheet_grounding": ("Vérification des sources", 7),
        "sheet_repair": ("Correction d'affirmations", 8),
        "sheet_workspace": ("Préparation du Workspace", 9),
        "sheet_done": ("Analyse terminée", 10),
    }
    ...
    def _emit_sheet_workspace(self, payload: dict[str, Any], conversation_id: str,
                              request_id: str, *, phase: str) -> None:
        """Publie le Workspace sur le bus d'événements.

        `phase="deterministic"` : données réelles disponibles, narration encore
        vide — l'interface peut s'ouvrir. `phase="final"` : la synthèse validée
        est jointe. Une panne du bus ne fait jamais échouer l'analyse.
        """
        try:
            self._core.events.emit("sheet.workspace", {
                "phase": phase,
                "conversation_id": conversation_id,
                "request_id": request_id,
                "analysis_workspace": payload,
            ...
            self._debug(f"[sheet] workspace emit failed phase={phase}")

    def _emit_sheet_summary(self, payload: dict[str, Any], conversation_id: str,
                            request_id: str) -> None:
        """Publie UNIQUEMENT la synthèse, une fois validée.

        L'interface a déjà les données : elle n'a plus qu'à remplir le bloc
        « Synthèse ». On n'envoie donc pas un second payload complet.
        """
        try:
            self._core.events.emit("sheet.summary", {
                "conversation_id": conversation_id,
                "request_id": request_id,
                "narrative": payload["summary"].get("narrative", ""),
                "headline": payload["summary"].get("headline", ""),
                "badge": payload["summary"].get("badge", ""),
                "grounding": payload.get("grounding", {}),
            })
        except Exception:
            self._debug("[sheet] summary emit failed")
