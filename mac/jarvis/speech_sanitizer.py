...
_EMOJI = re.compile(
    ...
    "\U000000A9\U000000AE"    # © ®
    "\U00002013-\U00002015"   # tirets – — ― (lus « moins » / « tiret »)
    "\U00002018-\U0000201F"   # apostrophes & guillemets courbes ' ' " " „ „
    "\U000000AB\U000000BB"    # guillemets « » (lus « guillemet »)
    "\U00002039\U0000203A"    # guillemets simples ‹ ›
    "\U00002020-\U00002023"   # obèles † ‡ et puces • ‣
    "\U00002043\U00002219\U000025E6\U000000B7"   # puces ⁃ ∙ ◦ · (interpoinçon)
    "]+"
)
...
def _int_words(n: int) -> str:
    ...
    if 21 <= n <= 99:
        # Le français est vigésimal au-delà de soixante : 71 se dit « soixante
        # et onze », pas « soixante-dix et un », et 91 « quatre-vingt-onze ».
        # On ramène donc 70-79 et 90-99 sur leur base (60 ou 80) plus un reste
        # de 10 à 19, qui a ses propres mots.
        base = 60 if 70 <= n <= 79 else 80 if 90 <= n <= 99 else (n // 10) * 10
        rest = n - base
        base_word = _SMALL_WORDS.get(str(base), str(base))
        if not rest:
            # « quatre-vingts » prend un s, sauf suivi d'un autre nombre.
            return base_word
        rest_word = _NUMBER_WORDS.get(str(rest)) or _SMALL_WORDS.get(str(rest)) or str(rest)
        # « quatre-vingts » perd son s dès qu'un nombre le suit.
        head = base_word[:-1] if base == 80 and base_word.endswith("s") else base_word
        # « vingt et un », « soixante et onze » — mais « quatre-vingt-un ».
        if (rest == 1 and base != 80) or (rest == 11 and base == 60):
            return f"{head} et {rest_word}"
        return f"{head}-{rest_word}"
    if 100 <= n <= 999:
...
def sanitize_for_speech(text: str) -> str:
    ...
    # Devises : jamais de symbole isolé laissé au TTS (« euro », « dollar »).
    t = t.replace("€", " euros").replace("£", " livres sterling").replace("$", " dollars")
    # Tirets isolés, barres obliques et points de suite : un TTS les lit par
    # leur nom (« moins », « slash », « point point point »). Les tirets À
    # L'INTÉRIEUR d'un mot (dix-sept) restent intacts : la règle exige un espace.
    t = re.sub(r"(\d)\s*/\s*(\d)", r"\1 sur \2", t)
    t = re.sub(r"/", " ", t)
    t = re.sub(r"\\", " ", t)
    t = re.sub(r"(?<=\s)-(?=\s)", " ", t)
    t = re.sub(r"[_]", " ", t)
...
PUBLIC_ACTIVITY_LABELS = (
    ...
    "Détection des tableaux", "Workspace prêt", "Analyse des données",
