...
class Tool:
    ...
    connector_types: tuple[str, ...] = ()     # ensemble accepté ; vide → (connector_type,)
    connector_optional: bool = False

    @property
    def accepted_connector_types(self) -> tuple[str, ...]:
        return self.connector_types or ((self.connector_type,) if self.connector_type else ())
    ...
    def public(self, core=None) -> dict[str, Any]:
        status = "ready"
        detail = ""
        types = self.accepted_connector_types
        if not self.enabled:
            status, detail = "disabled", "Désactivé dans les réglages"
        elif types and core is not None:
            available = [c for t in types for c in core.connectors.active(t)]
            if not available and not self.connector_optional:
                status = "not_configured"
                label = " ou ".join(types)
                detail = f"Aucun connecteur « {label} » configuré"
        return {
    ...
    def llm_schema(self) -> dict[str, Any]:
        ...
        if types:
            label = " ou ".join(types)
            desc += f" (nécessite un connecteur {label} ; passer connector_id)"
