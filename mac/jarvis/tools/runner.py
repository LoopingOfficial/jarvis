...
class SecureToolRunner:
    ...
    def _resolve_connector(self, tool: Tool, arguments: dict[str, Any]) -> dict[str, Any] | None:
        types = tool.accepted_connector_types
        if not types:
            return None
        core = self._core
        ...
        if requested:
            ...
            if found and found["type"] in types and found.get("enabled"):
                return found
            if not tool.connector_optional:
                label = " ou ".join(types)
                raise ToolDenied(
                    f"Aucun connecteur « {requested} » de type {label}. "
        ...
        if not available:
            ...
            label = " ou ".join(types)
            raise ToolDenied(
                f"Aucun connecteur {label} n'est configuré. "
                f"Ouvre Settings → Connectors pour en ajouter un."
            )
        names = ", ".join(f"{c['id']} ({c['name']})" for c in available[:6])
        raise ToolDenied(f"Plusieurs connecteurs sont disponibles, précise connector_id : {names}")
