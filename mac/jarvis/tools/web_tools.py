...
def _email_send(ctx: ToolContext) -> ToolResult:
    ...
    try:
        send_mail(ctx.connector["type"], ctx.config, ctx.secret("password"),
...
registry.add(
    ...
    description="Envoie un email via un connecteur email (SMTP+IMAP) ou SMTP.",
    handler=_email_send, connector_type="smtp", connector_types=("smtp", "email"),
    risk=SENSITIVE, permissions=("write",),
...
def _email_read(ctx: ToolContext) -> ToolResult:
    ...
    unread_only = bool(ctx.arguments.get("unread_only", True))
    try:
        messages = read_mail(ctx.connector["type"], ctx.config, ctx.secret("password"),
                             limit=limit, unread_only=unread_only)
        lines = [f"- {m.sender[:60]} — {m.subject[:80]}" for m in messages]
...
registry.add(
    ...
    handler=_email_read, connector_type="imap", connector_types=("imap", "email"),
    risk=READ_ONLY,
    input_schema={"type": "object", "properties": {
