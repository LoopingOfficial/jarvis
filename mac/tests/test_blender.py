...
class TestAnimate(BlenderE2EBase):
    ...
    def test_05_animate_spin(self):
        ...
        # « spin » est un clip d'objet : il produit une rotation nommée par son
        # axe (rotate_Z), pas par le nom du clip demandé.
        self.assertTrue(any("rotate" in a.lower() for a in anims),
                        f"rotation absente dans {anims}")
        print(f"ANIMATE SPIN OK  {job['test_elapsed']}s  anims={anims}")

    def test_06_animate_walk_needs_rig(self):
        """« walk » est un clip de personnage : sur un cube, il doit dire pourquoi.

        Le clip est désormais pris en charge (lib/animation.py) ; ce qui manque
        ici, c'est une armature. L'erreur doit le nommer explicitement plutôt
        que de prétendre que l'animation n'existe pas.
        """
        job = self._submit("animate", {
        ...
        self.assertIn("rig", err, f"le motif du refus n'est pas le rig : {err}")
        self.assertNotIn("non disponible", err,
                         f"« walk » est pris en charge, le refus ne doit pas dire l'inverse : {err}")
        print("ANIMATE WALK : refus motivé par l'absence de rig (erreur honnête)")

    def test_07_animate_unknown_clip_rejected(self):
        """Un clip qui n'existe nulle part reste refusé, avec la liste des clips."""
        job = self._submit("animate", {
            "clip": "moonwalk", "preview": False, "export_formats": [],
        }, reuse=True)
        self.assertEqual(job["status"], "failed")
        err = (job.get("error") or "").lower()
        self.assertIn("moonwalk", err)
        self.assertTrue(any(c in err for c in ("spin", "rotate", "bounce", "orbit")),
                        f"clips disponibles non listés : {err}")
        print("ANIMATE CLIP INCONNU : refusé avec la liste des clips")
