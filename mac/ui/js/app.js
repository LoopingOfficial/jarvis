...
goto(page, options = {}) {
    ...
    else if (page === 'blog') { /* Spatial V5 Blog module owns this page. */ }
  ...
  bindUI() {
    ...
    el('#convStop').onclick = () => this.cancelCurrent();
  ...
  updatePendingReply(text) {
    const nodes = this.pendingReplyNodes || [];
  ...
  async sendJarvisMessage(text, source = 'text', attachments = null) {
    ...
    return res;
      ...
      // Arrêt demandé par l'utilisateur : ce n'est pas une panne, on rend la
      // main sans bruit plutôt que d'afficher une erreur.
      if (error?.name === 'AbortError') {
        this.renderReply('Réflexion interrompue.', { ok: false, cancelled: true });
        VoiceManager.setState('IDLE', 'arrêt demandé');
      } else {
  ...
  /* ----------------------------------------------------------- annulation */
  // Le composer montre « Envoyer » au repos et « Arrêter » pendant que JARVIS
  // réfléchit : un seul emplacement, jamais les deux boutons en même temps.
  setBusy(on) {
    const send = $('#convSend');
    const stop = $('#convStop');
  ...
  // L'id de tâche n'existe qu'une fois l'orchestrateur démarré : il arrive par
  // le flux SSE (task.created) pendant que la requête est encore en vol. Si
  // l'utilisateur clique avant, on mémorise l'intention et on annule dès que
  // l'id est connu.
  async cancelCurrent() {
    ...
    this.activeTaskId = '';
    // Deux gestes complémentaires, car ils ne couvrent pas les mêmes cas :
    //  - côté serveur, l'orchestrateur teste l'annulation entre deux étapes,
    //    ce qui arrête les enchaînements d'outils encore à venir ;
    //  - côté client, on coupe l'attente tout de suite, car un appel au modèle
    //    déjà parti n'est pas interruptible et peut durer longtemps.
    if (taskId) J.post('/api/tasks/' + encodeURIComponent(taskId) + '/cancel', {});
    // La tâche serveur peut n'être annoncée qu'après l'arrêt local : on retient
    // l'intention pour l'annuler dès que son id arrive par le flux.
    else this.pendingServerCancel = true;
    this.abortCtl?.abort();
    // Le bandeau d'activité suit les événements serveur ; on le retire sans
    // attendre, l'utilisateur vient d'arrêter et doit le voir immédiatement.
    window.JarvisSpatial?.setTask?.('idle');
    toast('Réflexion interrompue.', 'ok');
  },
...
async showNodeDetails(node) {
  ...
  updateVoiceUI(state) {
    ...
    ($('#voiceStateLabel') || INERT).textContent = label;
    const mic = $('#micBtn') || INERT;
  ...
  sheetProgress(data) {
    ...
    if (data.stage === 'sheet_connect') this._sheetWorkspaceOpen = false;
    ...
    const brain = {
      ...
      sheet_semantic: 'THINKING', sheet_workspace_ready: 'SUCCESS', sheet_llm: 'THINKING',
    ...
    body.innerHTML = `<div class="sheet-progress">`
      + `<b>${esc(label)}</b>`
      + ` <span class="sheet-step">${data.step || 0}/${data.total || 10}</span>`
  ...
  /* ---- Workspace anticipe ------------------------------------------- */
  /* Les tableaux, KPI, previews et preuves proviennent tous de l'analyse
     deterministe. Ils sont disponibles bien avant la synthese du modele :
     on ouvre donc le Workspace immediatement, et on remplit la synthese
     quand elle arrive. */
  sheetWorkspaceReady(data) {
    const payload = data.analysis_workspace;
    if (!payload) return;
    if (data.phase === 'final') {
      // Payload complet et valide : il remplace la version anticipee.
      this._sheetWorkspaceOpen = false;
      return;
    }
    if (this._sheetWorkspaceOpen) return;      // deja ouvert pour cette analyse
    this._sheetWorkspaceOpen = true;
    this.openWorkspaceSafely(payload);
  },

  sheetSummaryReady(data) {
    // L'interface a deja les donnees : seule la narration manquait.
    const AW = window.AnalysisWorkspace;
    if (!AW || !AW.payload || !AW.payload.summary) return;
    AW.payload.summary.narrative = data.narrative || '';
    AW.payload.summary.pending_narrative = false;
    if (data.headline) AW.payload.summary.headline = data.headline;
    if (data.badge) AW.payload.summary.badge = data.badge;
    if (data.grounding) AW.payload.grounding = data.grounding;
    try {
      AW.renderMain();
    } catch (err) {
      console.debug('[sheet] rendu de la synthese differe impossible', err);
    }
  },
  ...
  bindStream() {
    ...
    J.on('llm.delta', (d) => this.updatePendingReply(d?.text || ''));
    J.on('sheet.progress', (data) => this.sheetProgress(data || {}));
    // Le Workspace s'ouvre des que les donnees deterministes sont pretes
    // (~3 s), sans attendre la synthese du modele (20-30 s).
    J.on('sheet.workspace', (data) => this.sheetWorkspaceReady(data || {}));
    // La synthese arrive ensuite et remplit le bloc deja affiche.
    J.on('sheet.summary', (data) => this.sheetSummaryReady(data || {}));
  ...
  routeEvent(e) {
    ...
    switch (e.type) {
      ...
      // --- tache de l'orchestrateur : porte l'id qui rend l'arret possible ---
      case 'task.created':
      case 'task.started':
        if (!d.id) break;
        if (this.pendingServerCancel) {
          this.pendingServerCancel = false;
          J.post('/api/tasks/' + encodeURIComponent(d.id) + '/cancel', {});
        } else if (this.sending) {
          this.activeTaskId = d.id;
        }
        break;
