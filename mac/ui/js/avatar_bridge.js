...
const AvatarBridge = {
  ...
  _bindVoice() {
    ...
    // Piper publie `tts.level` (amplitude 0..1). Garder aussi l'ancien nom
    // d'événement pour les lecteurs audio externes déjà branchés sur le bus.
    const applyAudioLevel = (d) => a.setAudioLevel(Number(d.level) || 0);
    J.on('tts.level', applyAudioLevel);
    J.on('tts.audio_level', applyAudioLevel);
    J.on('tts.completed', () => {
