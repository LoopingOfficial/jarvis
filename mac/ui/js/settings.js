...
const Settings = {
  ...
  SECTIONS: [
    ['general', 'General'], ['appearance', 'Appearance'], ['voice', 'Voice'], ['image', 'Image Generation'],
     ['ai', 'AI Providers'], ['blog', 'Blog / Discord'], ['connectors', 'Connectors'], ['blender', 'Atelier 3D'],
  ...
  async pane_voice(pane) {
    ...
    // Dictée serveur : on n'offre le mode local que s'il est réellement installé.
    let stt = {};
    try {
      const res = await J.get('/api/stt/status');
      if (res.ok !== false) stt = res;
    } catch { /* moteur indisponible */ }
    const sttProvider = v.stt_provider || 'browser';
    ...
    pane.innerHTML = `
      ...
      <div class="card" style="margin-bottom:11px"><div class="card-head"><h2>ÉCOUTE</h2></div><div class="card-body">
        ${this.field('Reconnaissance vocale', `<select data-k="stt_provider" data-stt-select>
          <option value="browser" ${sttProvider === 'browser' ? 'selected' : ''}>Navigateur (Web Speech API)</option>
          <option value="local" ${sttProvider === 'local' ? 'selected' : ''}>Locale (faster-whisper)</option>
        </select>`, sttProvider === 'local'
          ? (stt.available
              ? `Transcription sur ta machine · modèle « ${esc(stt.model || '')} »${stt.model_cached ? '' : ' (poids téléchargés au 1er usage)'}`
              : 'Moteur absent : installe-le avec « pip install faster-whisper ».')
          : (browserStt
              ? 'Dictée du navigateur : indisponible dans Firefox, et traitée à distance dans Chrome.'
              : 'Aucune dictée intégrée dans ce navigateur : bascule sur la reconnaissance locale.'))}
    ...
    // Bascule Navigateur ↔ Locale : l'aide affichée décrit le moteur choisi.
    const sttSelect = $('[data-stt-select]', pane);
    if (sttSelect) {
      const sttHint = sttSelect.parentElement.querySelector('.hint');
      sttSelect.onchange = () => {
        if (!sttHint) return;
        sttHint.textContent = sttSelect.value === 'local'
          ? (stt.available
              ? `Transcription sur ta machine · modèle « ${stt.model || ''} »`
                + (stt.model_cached ? '' : ' (poids téléchargés au 1er usage)')
              : 'Moteur absent : installe-le avec « pip install faster-whisper ».')
          : (browserStt
              ? 'Dictée du navigateur : indisponible dans Firefox, et traitée à distance dans Chrome.'
              : 'Aucune dictée intégrée dans ce navigateur : bascule sur la reconnaissance locale.');
  ...
  /* --------------------------------------------------------- BLOG / DISCORD */
  async pane_blog(pane) {
    const b = this.data.blog || {};
    const status = await J.get('/api/discord/status').catch(() => ({ blog_channel: {} }));
    const ds = status.blog_channel || {};
    const perms = ds.permissions || {};
    const check = (label, value) => `<div class="list-row"><div class="meta"><b>${label}</b></div><span class="tag ${value ? 'ok' : 'err'}">${value ? 'OK' : 'Non'}</span></div>`;
    pane.innerHTML = `<div class="card" style="margin-bottom:11px"><div class="card-head"><h2>BLOG / DISCORD</h2>
      <span class="tag ${status.connected ? 'ok' : ''}">${status.connected ? 'Connecté' : 'Non connecté'}</span></div><div class="card-body">
      <div class="field row"><div style="flex:1"><label style="margin:0">Bot Discord</label><div class="hint">Le token est enregistré uniquement dans le coffre sécurisé JARVIS.</div></div>
        <button class="btn sm primary" data-discord-connect>${status.connected ? 'Vérifier' : 'Connecter'}</button></div>
      ${this.field('Token bot (coffre sécurisé)', '<input type="password" data-discord-token autocomplete="new-password" placeholder="Laisser vide si déjà configuré">', 'Champ masqué. Le token n’est jamais renvoyé par l’API après sauvegarde.')}
      ${this.field('Salon articles', `<input class="mono" data-k="discord_channel_id" value="${esc(b.discord_channel_id || '1444705266825166972')}" inputmode="numeric"/>`, 'Aucun message de test ne sera envoyé.')}
      <div class="list" style="margin-top:8px">${check('Authentification', !!ds.authenticated)}${check('Serveur accessible', !!ds.guild_accessible)}${check('Salon accessible', !!ds.channel_accessible)}${check('ViewChannel', !!perms.view_channel)}${check('SendMessages', !!perms.send_messages)}${check('EmbedLinks', !!perms.embed_links)}</div>
      <button class="btn primary" data-save-blog>Enregistrer le salon</button>
    </div></div>
    <div class="card"><div class="card-head"><h2>AUTO_EDITORIAL</h2><span class="tag cy">mode sécurisé</span></div><div class="card-body">
      ${this.toggle('AUTO_EDITORIAL', 'auto_editorial', b.auto_editorial, { hint: 'Recherche, vérification, rédaction et brouillons. Jamais de publication automatique.' })}
      ${this.toggle('AUTO_RESEARCH', 'auto_research', b.auto_research)}
      ${this.toggle('AUTO_DRAFTS', 'auto_drafts', b.auto_drafts)}
      ${this.toggle('AUTO_PUBLISH', 'auto_publish', false, { locked: true, hint: 'Désactivé par défaut pour protéger le site.' })}
      ${this.toggle('AUTO_DISCORD', 'auto_discord', false, { locked: true, hint: 'Désactivé par défaut; aucun message automatique.' })}
      <div class="field row"><div style="flex:1"><label style="margin:0">Scheduler éditorial</label><div class="hint">Recherche éditoriale deux fois par jour · doublons et grounding insuffisant bloqués.</div></div><span class="tag ok">12 h</span></div>
      <button class="btn" data-run-editorial>Lancer une recherche maintenant</button>
    </div></div>`;
    this.bindToggles(pane, 'blog');
    $('[data-save-blog]', pane).onclick = () => this.save('blog', { discord_channel_id: $('[data-k="discord_channel_id"]', pane).value });
    $('[data-discord-connect]', pane).onclick = async (ev) => {
      ...
      if (token) {
        ...
        const payload = { type: 'discord', id: existing?.id || 'discord_bot', name: 'Bot Discord',
          config: { bot_token: token, default_channel: $('[data-k="discord_channel_id"]', pane).value } };
        if (existing) await J.put(`/api/connectors/${existing.id}`, payload);
        else await J.post('/api/connectors', payload);
        $('[data-discord-token]', pane).value = '';
      }
      const r = await J.post('/api/discord/connect'); toast(r.ok && r.blog_channel?.ok ? 'Bot et salon accessibles.' : (r.error || 'Vérification incomplète.'), r.ok && r.blog_channel?.ok ? 'ok' : 'err');
      this.renderPane(pane);
    };
    $('[data-run-editorial]', pane).onclick = async () => { const r = await J.post('/api/blog/auto/run'); toast(r.ok ? 'Cycle éditorial terminé.' : 'Cycle éditorial en échec.', r.ok ? 'ok' : 'err'); };
  },
...
const Connectors = {
  ...
  formHtml(type, connector) {
    ...
    const fields = spec.fields.map((f) => {
      ...
      if (f.kind === 'select') {
        const labels = (f.options_labels && f.options_labels.length) ? f.options_labels : f.options;
        return `<div class="field"><label>${esc(f.label)}</label>
          <select data-f="${esc(f.key)}">${f.options.map((o, i) =>
            `<option value="${esc(o)}" ${value === o ? 'selected' : ''}>${esc(labels[i] ?? o)}</option>`).join('')}</select></div>`;
      }
      if (f.kind === 'textarea') {
