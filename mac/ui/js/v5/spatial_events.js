...
(function () {
  ...
  const Events = {
    ...
    init(){
      ...
      // Arrêt demandé par l'utilisateur : ni succès ni échec, on retire le
      // bandeau tout de suite au lieu de le laisser tourner indéfiniment.
      J.on('task.cancelled', ()=>{ S()?.setTask('idle'); B()?.setState('IDLE'); });
