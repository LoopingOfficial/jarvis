...
const Home = {
  ...
  avatarCanvas: null, avatarParent: null, avatarNext: null, avatarShell: null,
  ...
  async mountViewer() {
    ...
    if (!stage) return;
    if (this.viewer) {
      this._moveAvatarTo(stage);
      return;
    }
    try {
      // L'accueil réutilise l'instance GLB du Command Center. Il ne doit ni
      // créer un hologramme différent ni télécharger un deuxième avatar.
      const started = Date.now();
      while (!window.JarvisAvatarSpatial && Date.now() - started < 8000) {
        await new Promise((resolve) => setTimeout(resolve, 80));
      }
      this.viewer = window.JarvisAvatarSpatial;
      if (!this.viewer?.canvas) throw new Error('avatar du Command Center indisponible');
      if (document.documentElement.getAttribute('data-context') !== 'home') return;
      this._moveAvatarTo(stage);
  ...
  _moveAvatarTo(stage) {
    if (!this.viewer?.canvas || !stage) return;
    if (!this.avatarCanvas) {
      this.avatarCanvas = this.viewer.canvas;
      this.avatarParent = this.avatarCanvas.parentElement;
      this.avatarNext = this.avatarCanvas.nextSibling;
      this.avatarShell = document.getElementById('v5Avatar');
    }
    stage.appendChild(this.avatarCanvas);
    this.avatarCanvas.style.width = '100%';
    this.avatarCanvas.style.height = '100%';
    if (this.avatarShell) this.avatarShell.style.display = 'none';
  },
  ...
  leave() {
    ...
    clearInterval(this.timer);
    this.timer = null;
    // Le même avatar retourne dans son emplacement du Command Center. On ne
    // détruit rien et on ne démarre jamais un second moteur 3D.
    if (this.avatarCanvas && this.avatarParent) {
      if (this.avatarNext && this.avatarNext.parentNode === this.avatarParent) {
        this.avatarParent.insertBefore(this.avatarCanvas, this.avatarNext);
      } else {
        this.avatarParent.appendChild(this.avatarCanvas);
      }
      this.avatarCanvas.style.width = '';
      this.avatarCanvas.style.height = '';
      if (this.avatarShell) this.avatarShell.style.display = '';
    }
