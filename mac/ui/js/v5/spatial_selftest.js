...
(function () {
  ...
  const SelfTest = {
    ...
    async run(){
      ...
      /* ------------------------------------------------- réglages / mode */
      // L'ancienne V4 a été supprimée : ce qu'on vérifie désormais, c'est
      // qu'aucun réglage d'interface ne subsiste pour la ressusciter, et
      // qu'elle n'est plus chargée du tout.
      let mode=null;
      try{ mode = localStorage.getItem('JARVIS_UI_MODE'); }catch(_){}
      this._('interface · aucun mode hérité persistant', mode===null, 'valeur = '+mode);
      this._('interface · couche V4 absente', typeof window.JarvisV4 === 'undefined');
      this._('interface · shell monté', document.documentElement.getAttribute('data-v5-ready')==='1');
