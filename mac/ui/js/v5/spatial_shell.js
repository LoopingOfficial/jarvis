...
(function () {
  ...
  // Les clés d'interface ne servent plus qu'à être EFFACÉES : la couche V4 a
  // été supprimée, il n'existe plus qu'une interface. On nettoie ce que les
  // versions précédentes ont pu laisser dans le stockage, sinon un
  // « legacy_v4 » oublié continuerait de masquer le shell.
  const LEGACY_KEYS = ['JARVIS_UI_MODE', 'JARVIS_UI_MODE_CHOSEN'];
  ...
  function readMode(){
    // Il n'y a plus qu'une interface. On purge les réglages devenus sans objet
    // pour qu'aucune machine ne reste bloquée sur un mode disparu.
    try{ for(const k of LEGACY_KEYS) localStorage.removeItem(k); }catch(_){ /* stockage indisponible */ }
    return 'spatial_v5';
  }
  ...
  const PAGE_CONTEXT = {
    ...
    analyses:'data', projects:'work', crm:'work', finance:'work', marketing:'work', social:'work',
  ...
  const SUBMENUS = {
    ...
    system:[['SYSTÈME'],['terminal','Terminal'],['aicore','AI Core'],['servers','Infrastructure'],['crm','CRM'],['finance','Finance'],['marketing','Marketing'],['social','Réseaux sociaux']],
  ...
  const Spatial = {
    ...
    boot(){
      ...
      try{
        ...
        console.error('[JARVIS UI V5] montage impossible', err);
        this.fallback(err);
        return false;
      }
    },
    ...
    /**
     * Échec de montage. Il n'existe plus de seconde interface vers laquelle
     * basculer : garder une V4 entière en réserve revenait à maintenir deux
     * produits pour un incident qui ne se produit presque jamais, et c'est
     * elle qui réapparaissait à l'écran en cas de pépin.
     *
     * On dit donc la vérité — ce qui a échoué, et comment relancer. Un écran
     * noir muet serait pire qu'une ancienne interface ; un message exact ne
     * l'est pas.
     */
    fallback(err){
      ...
      if(byId('v5BootError')) return;
      const panel=document.createElement('div');
      panel.id='v5BootError';
      panel.setAttribute('role','alert');
      panel.innerHTML = "<h1>Interface indisponible</h1>"
        + "<p>Le shell n’a pas pu se monter. Rien n’est perdu : le serveur et "
        + "les agents tournent toujours.</p>"
        + "<pre></pre>"
        + "<button type=\"button\">Relancer l’interface</button>";
      panel.querySelector('pre').textContent = String(err && (err.stack || err.message) || err || 'cause inconnue').slice(0, 600);
    ...
    async mountAvatar(){
      try{
        // L'avatar est procedural : plus aucun GLB, donc plus de rig importe,
        // plus de teinte a corriger apres coup et plus de cache a contourner.
        // La version dans l'URL reste utile : un module modifie continue sinon
        // d'etre servi depuis le cache du navigateur.
        const mod = await import('/js/avatar/holo_humanoid.js?v=HOLO_HUMANOID_1');
        const a = await mod.createHoloHumanoid({
          canvas: byId('v5AvatarCv'),
          framing: 'HALF_BODY',
        });
        this.avatar = a;
        window.JarvisAvatarSpatial = a;
      }catch(err){
